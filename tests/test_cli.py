    ...
    def test_main_validates_file_exists(self):
        ...
        with patch.object(sys, "argv", ["mcp-chain", "nonexistent_file.py"]):
            with patch.object(sys, "stderr", StringIO()) as mock_stderr:
                ...
                stderr_output = mock_stderr.getvalue()
                assert (
                    "Error: Chain definition file 'nonexistent_file.py' not found"
                    in stderr_output
                )
        ...
        try:
            with patch.object(sys, "argv", ["mcp-chain", temp_file]):
                with patch.object(sys, "stderr", StringIO()) as mock_stderr:
                    ...
                    assert (
                        "Variable that implements DictMCPServer protocol"
                        in stderr_output
                    )
        ...
        try:
            with patch.object(sys, "argv", ["mcp-chain", temp_file]):
                with patch.object(sys, "stderr", StringIO()) as mock_stderr:
                    ...
                    stderr_output = mock_stderr.getvalue()
                    assert (
                        "Error starting server: Server startup failed" in stderr_output
                    )
        finally:
            os.unlink(temp_file)
