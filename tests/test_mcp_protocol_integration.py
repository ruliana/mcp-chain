...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler(log_dir / "mcp_client.log"), logging.StreamHandler()],
)
...
class MCPProtocolClient:
    ...
    def send_request_and_get_response(
        self, method: str, params: Dict[str, Any] = None, timeout: int = 5
...
def test_mcp_protocol_initialization(mcp_server):
    ...
    init_response = client.send_request_and_get_response(
        "initialize",
        {
            ...
            "clientInfo": {"name": "mcp-protocol-test-client", "version": "1.0.0"},
        },
        timeout=10,
    )
    ...
    assert result["protocolVersion"] is not None, (
        "Server should return protocol version"
    )
...
def test_mcp_protocol_tools_discovery(mcp_server):
    ...
    for tool in tools:
        try:
            ...
            pytest.fail(
                f"Tool schema validation failed for {tool.get('name', 'unknown')}: {e}"
            )
...
def test_mcp_protocol_tool_execution(mcp_server):
    ...
    call_response = client.send_request_and_get_response(
        "tools/call",
        {"name": tool_name, "arguments": {"_args": ["Hello from MCP protocol test!"]}},
        timeout=10,
    )
    ...
    assert "content" in call_response["result"], (
        "Tool call response should contain 'content'"
    )
...
def test_mcp_protocol_middleware_logging(mcp_server):
    ...
    for i, line in enumerate(log_lines):
        try:
            ...
            assert log_entry["direction"] in ["request", "response"], (
                f"Log entry {i} has invalid direction"
            )
...
def test_mcp_protocol_error_handling(mcp_server):
    ...
    if error_response is None:
        ...
        assert error["code"] in expected_codes, (
            f"Should return method not found or invalid request error code, got {error['code']}"
        )
    ...
    try:
        error_response = client.send_request_and_get_response(
            "tools/call", {"name": "nonexistent_tool", "arguments": {}}, timeout=3
        )
    ...
    try:
        ...
        if response and "error" in response:
            ...
            expected_codes = [
                ...
                -32600,
                -32601,
            ]  # Invalid params, invalid request, or method not found
            assert response["error"]["code"] in expected_codes, (
                f"Should return appropriate error code, got {response['error']['code']}"
            )
    except Exception as e:
...
def test_mcp_protocol_complete_flow():
    ...
    try:
        ...
        init_success = (
            init_response is not None
            and "result" in init_response
            and "protocolVersion" in init_response["result"]
        )

        test_results.append(
            {"test": "initialize", "success": init_success, "response": init_response}
        )
        ...
        tools_success = (
            ...
            and "result" in tools_response
            and "tools" in tools_response["result"]
        )

        test_results.append(
            {"test": "tools_list", "success": tools_success, "response": tools_response}
        )
        ...
        if tools_success and len(tools_response["result"]["tools"]) > 0:
            ...
            call_response = client.send_request_and_get_response(
                ...
                {
                    "name": tool_name,
                    "arguments": {"_args": ["Hello from MCP protocol test!"]},
                },
                timeout=10,
            )

            call_success = (
                call_response is not None
                and "result" in call_response
                and "content" in call_response["result"]
            )
        ...
        if logging_success:
            ...
            logged_requests = sum(
                1 for line in log_lines if '"direction": "request"' in line
            )
            logged_responses = sum(
                1 for line in log_lines if '"direction": "response"' in line
            )
        ...
        results_file = (
            log_dir
            / f"mcp_protocol_test_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
...
def main():
    ...
    print(
        "  pytest tests/test_mcp_protocol_integration.py::test_mcp_protocol_complete_flow -v"
    )
    print()
