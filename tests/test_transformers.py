...
def test_middleware_with_dict_metadata_transformer():
    ...
    middleware = MiddlewareMCPServer(
        downstream_server=downstream, metadata_transformer=transform_metadata
    )

    result = middleware.get_metadata()
    expected = {
        "tools": [{"name": "test_tool", "description": "transformed: original"}]
    }
...
def test_middleware_with_dict_request_transformer():
    ...
    downstream = MockDictServer()
    middleware = MiddlewareMCPServer(
        downstream_server=downstream, request_transformer=transform_request
    )
...
def test_correct_chaining_order():
    ...
    chain = (
        mcp_chain()
        .then(raw_transform, lambda next_server, req: next_server.handle_request(req))
        .then(downstream)
    )
...
def test_mcp_chain_without_downstream_should_fail():
    ...
    chain_without_downstream = mcp_chain().then(
        dummy_transform, lambda next_server, req: next_server.handle_request(req)
    )
...
def test_transformer_controls_next_server_execution():
    ...
    def conditional_metadata_transformer(next_server, metadata_dict):
        ...
        if metadata_dict.get("test_condition"):
            ...
            return {
                "tools": [
                    {"name": "blocked_tool", "description": "blocked by condition"}
                ]
            }
    ...
    chain = (
        mcp_chain()
        .then(conditional_metadata_transformer, conditional_request_transformer)
        .then(downstream)
    )
...
def test_request_flow_with_collector():
    ...
    class CollectorServer:
        ...
        def get_metadata(self):
            ...
            return {
                "tools": [{"name": "collector_tool", "description": "original_desc"}]
            }
