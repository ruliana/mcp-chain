...
def test_then_with_porcelain_metadata_transformer():
    ...
    chained = (
        mcp_chain()
        .then(transform_metadata, identity_request_transformer)
        .then(downstream)
    )
...
def test_then_with_porcelain_request_transformer():
    ...
    chained = (
        mcp_chain()
        .then(identity_metadata_transformer, transform_request)
        .then(downstream)
    )
...
def test_then_with_raw_transformers():
    ...
    chained = (
        mcp_chain()
        .then(raw_metadata_transformer, raw_request_transformer)
        .then(downstream)
    )
    ...
    expected_metadata = {
        "tools": [{"name": "test_tool", "description": "raw_transformed: original"}]
    }
...
def test_chaining_multiple_middleware():
    ...
    expected_metadata = {
        "tools": [{"name": "test_tool", "description": "first: second: downstream"}]
    }
    assert metadata_result == expected_metadata
