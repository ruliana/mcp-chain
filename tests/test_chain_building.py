...
def test_mcp_chain_builder_replaces_itself_in_chain():
    ...
    step1 = mcp_chain().then(
        first_transformer, lambda next_mcp, req: next_mcp.handle_request(req)
    )
    # step1 is MiddlewareMCPServer(first_transformer) -> MCPChainBuilder

    step2 = step1.then(
        second_transformer, lambda next_mcp, req: next_mcp.handle_request(req)
    )
    ...
    assert (
        description == "first: second: test_tool"
    )  # first wraps (second wraps (downstream))
