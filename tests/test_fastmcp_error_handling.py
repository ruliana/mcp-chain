...
class MockDictMCPServer:
    ...
    def __init__(
        ...
        should_fail_metadata: bool = False,
        should_fail_request: bool = False,
    ):
...
def test_fastmcp_server_handles_fastmcp_initialization_failure(
    mock_fastmcp_class, caplog
):
...
def test_fastmcp_server_handles_duplicate_tool_names(caplog):
    ...
    assert (
        "Duplicate tool name 'duplicate_tool' found, skipping registration"
        in caplog.text
    )
...
def test_fastmcp_server_handles_duplicate_resource_uris(caplog):
    ...
    assert (
        "Duplicate resource URI 'test://resource' found, skipping registration"
        in caplog.text
    )
...
def test_serve_function_handles_fastmcp_server_creation_failure(
    mock_fastmcp_server_class, caplog
):
